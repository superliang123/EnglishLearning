...
<html>
  ...
  <form action="" id="form1">
    <table class="table table-hover">
    	...
    	<tr>
    		...
    		<td><a href="data/download?path=${record.path}">${record.title}</a></td>
    		
    		<td>${record.time}</td>
    		<td>${record.author}</td>
    		<td><a href="data/download?path=${record.path}">下载</a>
    		<c:if test="${role ne 'student' }">
    		&#12288<a href="javascript:if(confirm('确定要删除吗？')) del('${ record.id}');">删除</a></td>
    		</c:if>
    	</tr>
    	</c:forEach>
  ...
  <script type="text/javascript">
  		...
  		function edit(id){
			...
			$.post("data/goEdit",{"id":id},function(data){
		...
		function sou(currPage){
			...
			$.post("data/list",{"currPage":currPage,"soukey":$("#soukey").val()},function(data){
  		...
  		function del(id){
			...
			$.post("data/del",{"id":id},function(data){
		...
		function list(currPage){
			...
			$.post("data/list",{"currPage":currPage},function(data){
		...
		function sub(){
			if($("input[type='checkbox']:checked").length==0){
				...
				$.post("data/delMore",$('#form1').serialize(),function(data){
