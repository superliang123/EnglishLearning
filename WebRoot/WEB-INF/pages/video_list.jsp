...
<html>
  ...
  <form action="" id="form1">
    <table class="table table-hover">
    	...
    	<tr>
    		...
    		<td><a onclick="play('${ record.id}');">${record.title}</a></td>
    		
    		<td>${record.time}</td>
    		<td>${record.author}</td>
    		<td>
	    		<a onclick="play('${ record.id}');">播放</a>
	    		&#12288<a href="video/download?path=${record.path}">下载</a>
  ...
  <script type="text/javascript">
  		...
  		function play(id){
			...
			$.post("video/goPlay",{"id":id},function(data){
  		...
  		function del(id){
			...
			$.post("video/del",{"id":id},function(data){
		...
		function list(currPage){
				...
				$.post("video/list",{"currPage":currPage},function(data){
		...
		function sou(currPage){
			...
			$.post("video/list",{"currPage":currPage,"soukey":$("#soukey").val()},function(data){
		...
		function sub(){
			if($("input[type='checkbox']:checked").length==0){
				...
				$.post("video/delMore",$('#form1').serialize(),function(data){
